...
const GRAVITY_VECTOR: Vector3 = Vector3::new(0.0,0.0,physical_constants::STANDARD_ACCELERATION_OF_GRAVITY as Real);
...
impl Body {
    ...
    pub fn new(mass: Real, inertia: Matrix3, position: Vector3, velocity: Vector3, attitude: UnitQuaternion, rates: Vector3) -> Body {
    ...
    pub fn new_from_statevector(mass: Real, inertia: Matrix3, statevector: StateVector) -> Body {
    ...
    pub fn get_dcm(state: &StateVector) -> Matrix3 {
    ...
    pub fn get_dcm_body(state: &StateVector) -> Matrix3 {
    ...
    fn get_derivative(&self, state: &StateVector, forces: &Vec<Force>, torques: &Vec<Torque>) -> StateVector {
        let mut world_forces = GRAVITY_VECTOR * self.mass;
        ...
        let velocity_dot = state.velocity().cross(&state.rates()) + ( (Body::get_dcm(&state) * world_forces) + body_forces ) * (1.0/self.mass);
        
        let o_x = state.rates()[0];
        let o_y = state.rates()[1];
        let o_z = state.rates()[2];
        let qdot_matrix = na::Matrix4::new(
            0.0, -o_x, -o_y, -o_z,
            o_x,  0.0,  o_z, -o_y,
            o_y, -o_z,  0.0,  o_x,
            o_z,  o_y, -o_x,  0.0
        ...
        let attitude_dot = 0.5 * qdot_matrix * state.fixed_rows::<4>(6);
    ...
    pub fn step(&mut self, forces: &Vec<Force>, torques: &Vec<Torque>, delta_t: Real) {
        ...
        let k2 = self.get_derivative(&(self.statevector + k1 * delta_t/2.0), &forces, &torques);
        let k3 = self.get_derivative(&(self.statevector + k2 * delta_t/2.0), &forces, &torques);
        let k4 = self.get_derivative(&(self.statevector + k3 * delta_t),     &forces, &torques);
        
        self.statevector = self.statevector + delta_t/6.0 * (k1 + 2.0*k2 + 2.0*k3 + k4);
    }
    
}

/// Add state vector helpers to the Body struct
impl StateView for Body {
